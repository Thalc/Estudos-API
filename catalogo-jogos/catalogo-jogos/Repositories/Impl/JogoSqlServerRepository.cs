...
namespace catalogo_jogos.Repositories.Impl
{
    public class JogoSqlServerRepository : IJogoRepository
    {
        ...
        public async Task<List<Jogo>> ObterPorProdutora(string nome, string produtora)
        {
            var jogos = new List<Jogo>();
        ...
        {
            var comando = $"insert into Games (Id, Nome, Produtora, Preco) values ('{jogo.Id}', '{jogo.Nome}', '{jogo.Produtora}', {jogo.Preco.ToString().Replace(",", ".")})";
        ...
        {
            var comando = $"update Games set Nome = '{jogo.Nome}', Produtora = '{jogo.Produtora}', Preco = {jogo.Preco.ToString().Replace(",", ".")} where Id = '{jogo.Id}'";
